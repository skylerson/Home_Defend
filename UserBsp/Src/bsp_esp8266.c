...
#define ESP8266_Delay_ms(ms) vTaskDelay(ms)
 ...
 * 调用  ：被外部调用
 */
bool ESP8266_Data ( char * cmd, char * reply1, char * reply2, u32 waittime )
{    
	strEsp8266_Fram_Record .InfBit .FramLength = 0;               //从新开始接收新的数据包
...
uint8_t ESP8266_Shut_LinkServer( FunctionalState enumEnUnvarnishTx,ENUM_ID_NO_TypeDef ucId)
{
		char cStr [20];
	  bool bRet = false;
	...
	if ( enumEnUnvarnishTx )
	{
		...
		if ( ucId < 5 )
			sprintf ( cStr, "AT+CIPCLOSE=%d", ucId );
...
bool ESP8266_SendString ( FunctionalState enumEnUnvarnishTx, char * pStr, u32 ulStrLength, ENUM_ID_NO_TypeDef ucId )
{
	...
	else
	{
		...
		bRet = ESP8266_Data ( pStr, "SEND OK", 0, 1000 );
  }
...
unsigned char *ESP8266_GetIPD(unsigned short timeOut)
{
	...
	do
	{
		if(strEsp8266_Fram_Record .InfBit .FramFinishFlag)								//如果接收完成
		{
			ptrIPD = strstr((char *)strEsp8266_Fram_Record .Data_RX_BUF, "IPD,");				//搜索“IPD”头
